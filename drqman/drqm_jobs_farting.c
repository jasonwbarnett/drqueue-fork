...
static void dnj_koj_frame_farting_script_search (GtkWidget *button, struct drqmj_koji_farting *info);
static void dnj_koj_frame_farting_script_set (GtkWidget *button, struct drqmj_koji_farting *info);
static void dnj_koj_frame_farting_scene_search (GtkWidget *button, struct drqmj_koji_farting *info);
static void dnj_koj_frame_farting_scene_set (GtkWidget *button, struct drqmj_koji_farting *info);
static void dnj_koj_frame_farting_bcreate_pressed (GtkWidget *button, struct drqmj_dnji *info);
static void dnj_koj_frame_farting_menuitem_response (GtkWidget *menuitem, struct drqmj_koji_farting *info);


GtkWidget *
dnj_koj_frame_farting (struct drqm_jobs_info *info) {
  GtkWidget *frame;
  GtkWidget *vbox;
  ...
  /* Frame */
  frame = gtk_frame_new ("Farting job information");
  ...
  info->dnj.koji_farting.escene = entry;
  gtk_tooltips_set_tip(tooltips,entry,"File name of the farting scene file that should be rendered",NULL);
  gtk_box_pack_start (GTK_BOX(hbox2),entry,TRUE,TRUE,2);
  button = gtk_button_new_with_label ("Search");
  gtk_tooltips_set_tip(tooltips,button,"File selector for the farting scene file",NULL);
  gtk_box_pack_start (GTK_BOX(hbox2),button,FALSE,FALSE,2);
  g_signal_connect (G_OBJECT(button),"clicked",
                    G_CALLBACK(dnj_koj_frame_farting_scene_search),&info->dnj.koji_farting);
  ...
  info->dnj.koji_farting.eviewcmd = entry;
  gtk_entry_set_text(GTK_ENTRY(entry),KOJ_FARTING_DFLT_VIEWCMD);
  ...
  info->dnj.koji_farting.escript = entry;
  gtk_entry_set_text (GTK_ENTRY(entry),fartingsg_default_script_path());
  gtk_box_pack_start (GTK_BOX(hbox2),entry,TRUE,TRUE,2);
  button = gtk_button_new_with_label ("Search");
  gtk_tooltips_set_tip(tooltips,button,"File selector for the script directory",NULL);
  gtk_box_pack_start (GTK_BOX(hbox2),button,FALSE,FALSE,2);
  g_signal_connect (G_OBJECT(button),"clicked",
                    G_CALLBACK(dnj_koj_frame_farting_script_search),&info->dnj.koji_farting);
  ...
  g_signal_connect (G_OBJECT(rt_menuitem1),"activate",
                    GTK_SIGNAL_FUNC (dnj_koj_frame_farting_menuitem_response), &info->dnj.koji_farting);
  ...
  case KOJ_FARTING:
    g_signal_connect (G_OBJECT(button),"clicked",
                      G_CALLBACK(dnj_koj_frame_farting_bcreate_pressed),&info->dnj);
    break;
  default:
    fprintf (stderr,"What ?!\n");
    break;
...
GtkWidget *
jdd_koj_farting_widgets (struct drqm_jobs_info *info) {
  GtkWidget *table;
  GtkWidget *label;
  GtkAttachOptions options = (GtkAttachOptions)(GTK_EXPAND | GTK_SHRINK | GTK_FILL) ;
  char *labels[] = { "Scene:", info->jdd.job.koji.farting.scene,
                     "View command:", info->jdd.job.koji.farting.viewcmd,
                     NULL };
  char **cur;
...
static void
dnj_koj_frame_farting_scene_search (GtkWidget *button, struct drqmj_koji_farting *info) {
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION(dialog)->ok_button),
                      "clicked", GTK_SIGNAL_FUNC (dnj_koj_frame_farting_scene_set), info);
...
static void
dnj_koj_frame_farting_scene_set (GtkWidget *button, struct drqmj_koji_farting *info) {
  char buf[BUFFERLEN];
  char *p;
...
static void
dnj_koj_frame_farting_bcreate_pressed (GtkWidget *button, struct drqmj_dnji *info) {
  struct fartingsgi fartingsgi; /* Farting script generator info */
  char *file;
  
  // fix compiler warning
  (void)button;

  printf("%i\n", info->koji_farting.render_type);
  if ((info->koji_farting.render_type == 2) || (info->koji_farting.render_type == 1)) {
  	fartingsgi.render_type = info->koji_farting.render_type;
  } else {
  	fartingsgi.render_type = 1; /* we render animations per default*/
  }

  strncpy (fartingsgi.scene,gtk_entry_get_text(GTK_ENTRY(info->koji_farting.escene)),BUFFERLEN-1);
  strncpy (fartingsgi.scriptdir,gtk_entry_get_text(GTK_ENTRY(info->koji_farting.escript)),BUFFERLEN-1);

  if ((file = fartingsg_create (&fartingsgi)) == NULL) {
...
static void
dnj_koj_frame_farting_script_search (GtkWidget *button, struct drqmj_koji_farting *info) {
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION(dialog)->ok_button),
                      "clicked", GTK_SIGNAL_FUNC (dnj_koj_frame_farting_script_set), info);
  gtk_signal_connect_object (GTK_OBJECT (GTK_FILE_SELECTION(dialog)->ok_button),
                             "clicked", GTK_SIGNAL_FUNC (gtk_widget_destroy),
                             (gpointer) dialog);
  gtk_signal_connect_object (GTK_OBJECT (GTK_FILE_SELECTION(dialog)->cancel_button),
...
static void
dnj_koj_frame_farting_script_set (GtkWidget *button, struct drqmj_koji_farting *info) {
  char buf[BUFFERLEN];
  char *p;
...
static void
dnj_koj_frame_farting_menuitem_response (GtkWidget *menuitem, struct drqmj_koji_farting *info) {
